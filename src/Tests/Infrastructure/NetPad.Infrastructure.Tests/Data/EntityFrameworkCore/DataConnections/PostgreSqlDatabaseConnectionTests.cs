...
public class PostgreSqlDatabaseConnectionTests : CommonTests
{
    ...
    public static IEnumerable<object?[]> ConnectionStringTestData => new[]
    {
        ...
        new[] { "host", "port", "db name", null, "password", "Host=host:port;Database=db name;UserId=;Password=password" },
        new[] { "host", "port", "db name", "user id", null, "Host=host:port;Database=db name;UserId=user id;Password=" },
