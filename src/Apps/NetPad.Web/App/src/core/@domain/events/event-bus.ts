...
export interface IEventBus extends IEventAggregator {
     ...
     */
    subscribeRemote<TMessage extends Constructable>(channel: string | TMessage, callback: (message: InstanceType<TMessage>, channel: string) => void): IDisposable;
}

export const IEventBus = DI.createInterface<IEventBus>();

export class EventBus extends EventAggregator implements IEventBus {
    public subscribeRemote<TMessage extends Constructable>(channel: string | Constructable, callback: (message: InstanceType<TMessage>, channel: string) => void): IDisposable {
