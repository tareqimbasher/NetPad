    ...
    where TResponse : class
{
    ...
    )
    {
        ...
        if (!typeof(CommandBase).IsAssignableFrom(typeof(TRequest)) &&
            !typeof(QueryBase).IsAssignableFrom(typeof(TRequest)))
        {
            throw new InvalidOperationException(
                $"{typeof(TRequest)} must inherit from {nameof(QueryBase)} or {nameof(CommandBase)}");
        }
